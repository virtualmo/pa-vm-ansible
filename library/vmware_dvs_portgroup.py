...
#     2017, Mohanad Elamin <melamin () paloaltonetworks.com>
#
...
#
#
# Update the module to all setting security policy for the dvs pg
# Mohanad Elamin
#
#
...
class VMwareDvsPortgroup(object):
    def __init__(self, module):
        ...
        self.promiscuous_mode = self.module.params['network_policy'].get('promiscuous_mode', None)
        self.forged_transmits = self.module.params['network_policy'].get('forged_transmits', None)
        self.mac_changes = self.module.params['network_policy'].get('mac_changes', None)
    ...
    def create_port_group(self):
        ...
        config.defaultPortConfig.securityPolicy.allowPromiscuous = vim.BoolPolicy(value=self.promiscuous_mode)
        config.defaultPortConfig.securityPolicy.forgedTransmits = vim.BoolPolicy(value=self.forged_transmits)
        config.defaultPortConfig.securityPolicy.macChanges = vim.BoolPolicy(value=self.mac_changes)
